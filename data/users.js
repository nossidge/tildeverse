...
function users(tildeverseJSON) {
  ...
  let thead = "<thead><tr><th>Tilde Box</th><th>User Name</th><th>User URL</th><th>Modified</th></tr></thead>";
  let tbody = document.createElement("tbody");
  $("#table_users").append(thead);
  $("#table_users").append(tbody);

  let row = '<tr><td><a href="SITE_URL">SITE_NAME</a></td><td>USER_NAME</td><td><a href="USER_URL">USER_URL_TIDY</a></td><td>MODIFIED</td></tr>';
  ...
  for (let tildeSite in tildeverseJSON["sites"]) {
    ...
    $.each(site["users"], function(user, hash) {
      ...
      let time = hash["time"].substring(0, 10);
      let out = row;
